...
export default function LineChart({ scout_history_data, data, entries_data }) {
  ...
  useEffect(() => {
    const updatedConcatEntryData = data
    .map(jobseeker => {
      const matchingEntry = entries_data.find(entries_data => jobseeker[4] === entries_data[3]);
      // 一致する jobSeeker が見つかった場合のみ結合
      return matchingEntry ? { ...jobseeker, ...matchingEntry } : undefined;
    })
    .filter(item => item !== undefined); // 一致する jobSeeker があるエントリのみ残す
    setConcatEntryData(updatedConcatEntryData);

    //年収でフィルター
    const salaryRanges1 = getSalaryRanges();
    let filteredJobseekerData = [];
    let filteredData = [];
    let rate = [];

    for (const range of salaryRanges1) {
      filteredJobseekerData[range] = filterDataBySalaryRange(data, ...getSalaryRangeValues(range));
      filteredData[range] = filterDataBySalaryRange(updatedConcatEntryData, ...getSalaryRangeValues(range));
    }
  ...
  const updatedConcatData = scout_history_data
    ...
    updateSalaryData(updatedConcatData, salaryRanges);

  }, [data, scout_history_data, entries_data]);

  function getInitialSalaryData() {
    return getSalaryRanges().reduce((acc, range) => {
  ...
  function getSalaryRanges() {
    return ['200', '200-300', '300-400', '400-500', '500-600', '600-700', '700-800', '800'];
  }
  ...
  function filterDataBySalaryRange(data, min, max) {
    return data.filter(item => item[11] >= min && item[11] <= max);
  }
